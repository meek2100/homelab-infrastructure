...
4. **Decryption Key**: Ensure your `master-age-key.txt` is placed in the root of this repository so the AI can decrypt Docker `.env` files.
...
3. **`restore_stacks`**: The AI will decrypt your SOPS secrets, write your `docker-compose.yml` files, and ignite all your Docker containers!
...
I wrote and executed a Python migration script that gracefully moved all 1,096 of your currently backed-up files out of the legacy `nodes/` structure and into a beautiful, human-readable GitOps layout.

> [!SUCCESS] Human-Readable Server Names
> You no longer need to memorize arbitrary IDs like `vm-100`. Your files are now perfectly categorized by their full server names exactly as you requested!
> - `infrastructure/vms/pve-100-nexus-server/drift`
> - `infrastructure/vms/pve-102-luna-server/drift`
> - `infrastructure/docker-stacks/media-server/plex/`

## 2. 🧠 The Proxmox Recovery MCP Server
I built a brand new Python Model Context Protocol (MCP) server in `mcp-servers/proxmox-recovery-mcp/` and registered it globally in your `.agents/mcp.json` file.
